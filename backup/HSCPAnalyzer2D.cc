...
//   const bool use_Digis = true;
  ...
  // Local variables 
  bool ydouble[TYSIZE], xdouble[TXSIZE], bpix;
  static float qscale, qscaleB, qscaleF, pcut, tkpcut, probQ2D, xs, ys, probQonTrack2D, probQonTrackTerm2D;
  static float probQonTrackWMulti2D = 1 ;
  static float xhit, yhit, xrec2D, yrec2D, sigmax2D, sigmay2D, probx, proby, cotalpha, cotbeta, locBx, locBz, xoff, yoff, xtemp, ytemp; 
  float deltay, probXY2D;
  ...
  static int lumiMin = 100000, lumiMax = 0, edgeflagy = 0, edgeflagx = 0;
  static vector<int> nbin(5,0);
  int i, j, ierr, qbin2D, ID;
  double log10probXY2D, log10probQ2D, log10probXYQ2D, logprobQonTrackWMulti2D, qclust, qnorm, proba, probXYQ, dx, dy, TkP, xhmod, yhmod;
...
//  chain.Add("4pixelTree-signal5000event.root");
//  chain.Add("4pixelTree_Signal.root"); 
//    chain.Add("4pixelTree.root"); 
//    chain.Add("4pixelTree-SimHitInfo.root");
  // Template reading
...
//        if(qbin > 3) {printf(" qbin = %d \n", qbin);}
            if (debug>2) cout << "probQ2D in cluster " << iTkCl << " is " << probQ2D <<endl;
            ...
            log10probXYQ2D = log10(probXYQ);
            hp[5]->Fill(probXY2D);
            hp[6]->Fill((double)probQ2D);
            
            
            hp[7]->Fill(probXYQ);
            hp[8]->Fill(log10probXY2D);
...
//             if(probQonTrackWMulti2D > 0.00000001) cout << "log probQonTrackWMulti2D " << log(probQonTrackWMulti2D) << endl;
        ...
        if(nprobQOnTrack2D < 2 || nprobQOnTrack2D!=TkClN) continue;
	if (debug>2) cout << "nprobQOnTrack2D: " << nprobQOnTrack2D << endl;
        ...
        hp[30]->Fill(probQonTrackWMulti2D);
        logprobQonTrackWMulti2D = log(probQonTrackWMulti2D);
        if(probQ2D > 0.00001) hp[31]->Fill(logprobQonTrackWMulti2D);
...
//             cout << "For cluster " << iTkCl << " pow(-logprobQonTrackWMulti2D,iTkCl) is " << pow(-logprobQonTrackWMulti2D,iTkCl) << endl;
            if (debug>1) cout << "For cluster " << iTkCl << " probQonTrackTerm2D is " << probQonTrackTerm2D << endl;
        }
          
        probQonTrack2D = probQonTrackWMulti2D*probQonTrackTerm2D;
        if (debug>0) cout << " probQonTrack " << probQonTrack2D << endl;
        if (debug>3) if (probQonTrack2D>0.98) cout << "probQonTrack is more then 0.98 in event " << event << endl;;
        hp[32]->Fill(probQonTrack2D);
      } // end of loop over tracks
   } //end cycle for events
//   cout << " and nHSCP_1 is " << nHSCP_1 << endl;
...
//  Create an output filename for this run 
   sprintf(outfile0,"Plots_SignalWithTracks_2D_New.pdf[");
   sprintf(outfile1,"Plots_SignalWithTracks_2D_New.pdf");
   sprintf(outfile2,"Plots_SignalWithTracks_2D_New.pdf]");
//    sprintf(outfile0,"Plots_NormalTracks_2D_New.pdf[");
//    sprintf(outfile1,"Plots_NormalTracks_2D_New.pdf");
//    sprintf(outfile2,"Plots_NormalTracks_2D_New.pdf]");
