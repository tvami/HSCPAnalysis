...
// Make sure that we find at least one pixel   
   if(npixel < 1) {
#ifndef SI_PIXEL_TEMPLATE_STANDALONE
      throw cms::Exception("DataCorrupt") << "PixelTemplateReco3D::number of pixels above threshold = " << npixel << std::endl;
#else
      std::cout << "PixelTemplateReco3D::number of pixels above threshold = " << npixel << std::endl;
#endif
      return 1;
   }
...
//    std::cout << "scalex: " << scalex << std::endl;
   ...
   if(id > 0) {
      ...
      float xvav_corr = (qtotal/qtfrac0[ipass]-mpv)/sigmaQ;
      float xvav = (qtotal-mpv)/sigmaQ; // should not apply the correction, #ToDo
//       std::cout << "xvav_corr vs xvav: " << xvav_corr <<" vs "<< xvav <<std::endl;
      float beta2 = 1.f;
